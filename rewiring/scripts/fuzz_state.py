...
"""
class Fuzz_Run:
    """Optimized fuzzing engine for SystemVerilog module interconnections."""
    
    def __init__(self, flattened_lib_path: Union[str, Path]):
        """Initialize the fuzzing engine with module library."""
        self.flattened_lib_path = flattened_lib_path
        io_map = IO_map.create_IO_map(flattened_lib_path)

        # Combine input and output wires into single source of truth
        self.all_wires: Dict[int, IO_map.Wire] = {**io_map['bit_input'], **io_map['bit_output']}
        
        self.mod_IO: Dict[str, Dict[str, Set[int]]] = io_map['mod_IO']
        self.all_modules: List[str] = io_map['mod_list']

        # Track external connections
        self.external_inputs: List[int] = list(io_map['bit_input'].keys())
        self.external_outputs: List[int] = list(io_map['bit_output'].keys())

        # Initialize module tree with better structure
        self.module_tree: Dict[str, Dict] = {}
        for i, module in enumerate(self.all_modules):
            self.module_tree[module] = {
                "depth": 0, 
                "tree": i, 
                "children": [], 
                "parents": [], 
    ...
    def linear_rewire(self) -> int:
        """
        Optimized linear rewiring with improved performance and validation.
        
        Returns:
            0 if successful rewiring occurred, 1 if no valid opportunity found
        """
        ...
        for _ in range(num_outputs):
            ...
            if output_wire.type not in (IO_map.Wire.Wire_Type.MTM, IO_map.Wire.Wire_Type.MTO):
                continue

            output_port = output_wire.input
            if "_output_" not in output_port:
                continue

            output_mod = output_port.split("_output_", 1)[0]

            # Skip if this wire is registered and belongs to a loop
            if output_wire.reg and output_mod in self.loop_set:
                continue

            # Cache module tree data for performance
            output_tree_data = self.module_tree[output_mod]
            output_tree = output_tree_data["tree"]
            output_depth = output_tree_data["depth"]
            ...
            for _ in range(num_modules):
                ...
                target_tree_data = self.module_tree[target_mod]
                target_tree = target_tree_data["tree"]
                target_depth = target_tree_data["depth"]
                ...
                # 2. Skip if direct parent-child already exists (use cached data)
                if (target_mod in output_tree_data["parents"] or 
                    target_mod in output_tree_data["children"] or 
                    output_mod in target_tree_data["parents"] or 
                    output_mod in target_tree_data["children"]):
                    continue
    ...
    def generate_top_module(self, output_path: Union[str, Path], top_name: str) -> str:
        """
        ...
        Args:
            output_path: Directory to write the top module file
        ...
        # Write to file
        final = "\n".join(lines)
        output_file_path = output_dir / f"{top_name}.sv"
